...
namespace Algorithm.Diff
{
    public class Merge : IEnumerable
    {
        ...
        public class Hunk : Algorithm.Diff.Hunk
        {
            ...
            {
                if (Conflict)
                    throw new InvalidOperationException ("ChangedIndex cannot be called if there is a conflict.");
                for (int i = 0; i < hunks.Length; i++) {
                    foreach (Diff.Hunk h in hunks[i])
                        if (!h.Same)
                            return i;
            ...
            {
                return new Range (
                    merge.diffs[index].Right,
                    hunks[index][0].Right.Start,
                    hunks[index][hunks[index].Length-1].Right.End - hunks[index][0].Right.Start + 1
                );
            }
