...
namespace Algorithm.Diff
{
            ...
            foreach (Diff.Hunk hunk in diff) {
                if (hunk.Same || (hunk.Left.Count == 1 && hunk.Right.Count == 1)) {
                    ...
                    while (leftitems.Count > 0 && rightitems.Count > 0) {
                        ...
                        CompareLists (
                            leftitems.GetRange(0, li),
                            rightitems.GetRange(0, ri),
                            comparer.minimumDifference,
                            output
                        );
                                                
                        // Then recurse on the correspondence range.
                        CompareLists (
                            leftitems.GetRange(li, count),
                            rightitems.GetRange(ri, count),
                            comparer.minimumDifference,
                            output
                        );
    ...
    public class XmlOutputStructuredDiff : StructuredDiff
    {
        ...
        protected override void WriteNodeSame (NodeInterface nodeInterface, object left, object right)
        {
            ...
            ((XmlOutputNodeInterface) nodeInterface).WriteBeginNode (
                left,
                left,
                (XmlWriter)output
            );
            ...
            if (deep)
                ((XmlOutputNodeInterface) nodeInterface).WriteNodeChildren (
                    left,
                    (XmlWriter)output
                );
        ...
        protected override void WriteNodeChange (NodeInterface leftInterface, object left, NodeInterface rightInterface, object right)
        {
            ...
            ((XmlOutputNodeInterface) leftInterface).WriteBeginNode (
                left,
                left,
                (XmlWriter)output
            );
            ((XmlOutputNodeInterface) leftInterface).WriteNodeChildren (
                left,
                (XmlWriter)output
            );
            output.WriteEndElement ();
            ((XmlOutputNodeInterface) rightInterface).WriteBeginNode (
                right,
                right,
                (XmlWriter)output
            );
            ((XmlOutputNodeInterface) rightInterface).WriteNodeChildren (
                right,
                (XmlWriter)output
            );
            output.WriteEndElement ();
    ...
    internal class NodeComparerWrapper : IComparer
    {
        ...
        private class Pair
        {
            ...
            public Pair (object a, object b, StructuredDiff differ)
            {
                left = a;
                right = b;
                code = unchecked(differ.GetInterface (left).GetHashCode (left) + differ.GetInterface (right)
                    .GetHashCode (right));
            }
